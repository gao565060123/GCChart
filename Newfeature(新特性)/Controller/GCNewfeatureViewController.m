...
#import "GCNewfeatureViewController.h"
#import "GCTabBarViewController.h"
#import "Header.h"

#define GCNewfeatureImageCount 3

@interface GCNewfeatureViewController () <UIScrollViewDelegate>
@property (nonatomic, weak) UIPageControl *pageControl;
@end

@implementation GCNewfeatureViewController
- (void)viewDidLoad
{
    [super viewDidLoad];
...
- (void)setupPageControl
{
    ...
    pageControl.numberOfPages = GCNewfeatureImageCount;
    ...
    // 2.设置圆点的颜色
    pageControl.currentPageIndicatorTintColor = GCColor(253, 98, 42);
    pageControl.pageIndicatorTintColor = GCColor(189, 189, 189);
}

/**
 *  添加UISrollView  添加软件新特性的图片
 */
- (void)setupScrollView
{
    ...
    for (int index = 0; index<GCNewfeatureImageCount; index++) {
        ...
        // 在最后一个图片上面添加按钮
        if (index == GCNewfeatureImageCount - 1) {
            [self setupLastImageView:imageView];
        }
    }
    
    // 3.设置滚动的内容尺寸     滚动范围
    scrollView.contentSize = CGSizeMake(imageW * GCNewfeatureImageCount, 0);
    scrollView.showsHorizontalScrollIndicator = NO;   // 水平滚动条
    scrollView.pagingEnabled = YES;                              // 分页
    scrollView.bounces = NO;                                            // 弹簧效果 关闭
}
...
- (void)setupLastImageView:(UIImageView *)imageView
{
    ...
    /**********************************************************************************/
    // 4.添加checkbox   ----->  是否分享给大家   是两张图片     默认必须是yes
...
- (void)start
{
    ...
    // 切换窗口的根控制器
    self.view.window.rootViewController = [[GCTabBarViewController alloc] init];
}
